...
Read `${CLAUDE_PLUGIN_ROOT}/skills/prompt-journal/SKILL.md` and follow it exactly.

Selector — **omit to analyse the whole journal** (default `~/.claude/prompt-journal/prompts`); or
narrow to a single file/dir path, `--project <name>`, or `--branch <name>`:
