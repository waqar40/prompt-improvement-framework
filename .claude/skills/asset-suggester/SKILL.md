...
- `results` (optional): this run's prompt-critic results, each with its prompt, source,
  `project`, `root`, `branch`, date, `top_dimensions`, and any `asset_hint`.
- `user` (required): whose store/suggestions to update (e.g. `waqar.aziz`).
- `store` (optional): `scores/<user>.jsonl` — the full history (now carries `project`/`root` per
  row); read it so recurrence is measured across the whole journal, not just this run.
...
tools/verbs used, and any `asset_hint`. An `asset_hint` is a *shape* flag, not proof of recurrence.
