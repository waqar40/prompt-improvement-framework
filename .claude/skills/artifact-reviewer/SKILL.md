...
description: Use to REVIEW existing Claude Code assets — a skill, subagent, hook, slash command, memory rule, or script (one asset or a whole .claude/ tree) — against the same quality gate we build to. Runs the deterministic frontmatter validator, then audits anatomy, the 7 rubrics (correctness/latency/cost/security/observability/scale/reliability), the non-destructive permission posture, model-tier fit, and whether it ships a verification; returns a severity-ranked findings report + score + PASS/FAIL, with a concrete fix per finding. Read-only — it never edits (fixes are routed to asset-architect or a human). Triggers include "review this skill/agent/hook", "audit my .claude assets", "does this skill follow best practices", "quality-check this command", "review-asset", and the /review-asset command.
...
## Inputs
- `target` (optional): an asset file, an asset dir, or a repo root. **Default: `.claude/`** of the cwd.
...
For each asset, read it (and, for grounding claims, the files/commands it references) and walk the
quality gate sections **B–F**: anatomy/structure & correct type · the 7 rubrics · permission posture
(destructive ops denied? read-only roles actually read-only?) · model-tier fit · verification present.
Cite a concrete `file:line` for every finding; **never fabricate** one. Judge like an adversarial
...
blocking[], major[], minor[], rubric_coverage{7}}` then a short markdown summary — each finding as
`file:line — issue — fix`. For a tree, add a leaderboard (worst gate failures first) and totals.
**Gate = FAIL** if any blocking item or an unaddressed rubric.

### Step 5 — Route fixes (do not apply)
List the fixes, grouped blocking→minor. For anything to be changed, point to **`/scaffold-asset`**
(asset-architect applies fixes after approval) or hand back to the human. Never edit an asset here.

## Constraints
- READ-ONLY — NEVER Edit/Write/scaffold an asset; you review, `asset-architect` builds/fixes.
- ALWAYS run the deterministic validator first, then the judgment gate; keep both consistent with
  `quality-gate.md` — do not invent criteria.
...
- Verification (this skill's own eval): a fixture asset with known defects must score `FAIL` with those
  exact findings, and a known-good asset must score `PASS`.
