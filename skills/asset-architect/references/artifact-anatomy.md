...
- **Verification (EDD).** Ship `evals/evals.json` (schema + grader types: `verification-harness.md`)
  — realistic prompts run with the skill vs. with it disabled, in fresh sessions, graded pass/fail;
  measure *did it fire* AND *was the output right* separately. (`skill-creator` automates this.)
