...
- [ ] the artifact **ships its own check** per the minimum-per-type table in `verification-harness.md` — skill `evals/`; agent output-contract + adversarial review; hook sample-payload exit-code test; command dry-run; rule adherence check; script self-test — *blocking if absent*
- [ ] where `evals/evals.json` exists, each case names ≥1 grader from `verification-harness.md`'s grader table — *major if present but empty/case-less*

## G. Instructional semantics & composition (per `semantic-consistency.md`)

- [ ] **G1 contradiction** — no instruction in the artifact (or across it and a file it composes with) that conflicts with another — *blocking*
- [ ] **G2 semantic ambiguity** — no unresolvable pronoun/vague success word/unstated trigger on a load-bearing instruction — *major, blocking if on a destructive/write gate*
- [ ] **G3 persona consistency** — an agent's stated role/tone holds for its whole body and matches its tool grant — *major, agents only (n/a elsewhere)*
- [ ] **G4 cognitive load** — no >3-level nested conditional prose, no steps-that-aren't-sequential, a scannable Constraints block — *major, minor if shallow*
- [ ] **G5 semantic coverage** — every input/edge case the description promises is handled by a Step; reachable error paths aren't silently unhandled — *major if reachable in normal use, minor if edge-case*
...
`asset-fixer`; anything needing authored content or redesign to `asset-architect` or the human.
