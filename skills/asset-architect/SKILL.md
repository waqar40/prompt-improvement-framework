...
| `references/verification-harness.md` | The concrete `evals/evals.json` schema + grader types backing gate Section F — the shape "ships its own check" takes. |
...
to verify it** (EDD): a skill gets `evals/evals.json` (schema: `references/verification-harness.md`),
a subagent an output contract, a hook a sample-payload exit-code test, a script a self-test.
**Run the shared quality gate on your own draft** (`references/quality-gate.md`, incl.
`scripts/validate-frontmatter.py` for the frontmatter slice AND `references/semantic-consistency.md`
for Section G — read your own draft as an adversary would, including every file your `references/`
table or "follow `<skill>.md`" line points to) — it must reach `PASS` (zero blocking, all 7 rubrics
addressed, Section G checked) before you present. **Present the draft + target path + the
verification + the quality-gate result (score + any remaining items) for approval — do not write yet.**
