...
| `prompt-journal` | "run the prompt pipeline", "analyse all my prompts" (the engine behind `/analyse`) | **Default: the whole journal** (`~/.claude/prompt-journal/prompts`), or a file/`--project`/`--branch` + a user | Orchestrates score → per-file review → store → **overall** guide → machine-readable suggestions, treating entries under one `branch=` header as one session/chain. |
...
read `~/.claude/prompt-journal/prompts-review-outcomes/guides/<you>.md` → optionally
`/scaffold-asset` a recurring pattern.
